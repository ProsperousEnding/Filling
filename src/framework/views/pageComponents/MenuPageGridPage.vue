    ...
    >
      <template v-if="usesArticleCard(item)">
        ...
        <div
          ...
          class="menu-page-grid-cover-placeholder h-40 md:h-44"
          :data-placeholder="coverListConfig.placeholder"
        >
          <svg v-if="coverListConfig.placeholder === 'icon'" xmlns="http://www.w3.org/2000/svg" class="h-9 w-9" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
const containerStyle = computed(() => {
  ...
  return {
    ...(normalizedColumns ? { '--menu-page-grid-columns': String(normalizedColumns) } : {}),
    ...(normalizedWideColumns ? { '--menu-page-grid-wide-columns': String(normalizedWideColumns) } : {})
  }
})
...
.menu-page-grid-cover-placeholder {
  display: flex;
  align-items: center;
...
.menu-page-grid-cover-placeholder[data-placeholder='icon'] {
