<template>
  <div class="tag-cloud">
    <!-- 标题 -->
    <h3 v-if="title" class="tag-cloud-title theme-section-title text-lg font-medium mb-4">{{ title }}</h3>
    ...
    <div v-else-if="tags.length === 0" class="tag-cloud-empty text-center py-4">
      暂无标签
    </div>
    
    <div v-else class="tag-cloud-list flex flex-wrap gap-2">
      <router-link 
        v-for="tag in tags" 
        :key="tag.id" 
        :to="getTagRoute(tag)" 
