<template>
  <div class="article-feed-item article-item">
    <div
      ...
      :class="hasArticleCover(article) ? 'article-feed-card-with-cover' : 'article-feed-card-without-cover'"
      :style="getArticleBackgroundStyle(article)"
    >
      <div class="article-feed-card-overlay absolute inset-0"></div>

      <div v-if="!hasArticleCover(article)" class="article-feed-card-fallback absolute inset-0 flex items-center justify-center">
...
const hasArticleCover = (article) => Boolean(article?.imageUrl || article?.cover)

const getArticleBackgroundStyle = (article) => (
  hasArticleCover(article)
    ? {
